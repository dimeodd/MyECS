...
namespace MyEcs
{
    public class EcsWorld
    {
        ...
        //FIXME сделать команду, которая будет уменьшать Component-массивы. и вызывать её раз в ~10 цыклов у системы

        public const int POOL_SIZE = 8;
        public const int FILTER_LINKS_SIZE = 4;
        ...
        public EcsWorld()
        {
            ...
            entityData = new EntityData[POOL_SIZE];
            freeEntity = new MyList<int>(POOL_SIZE);
            pool = new IComponentPool[POOL_SIZE]; //это массив интерфейсов
        }
        ...
        internal ComponentPool<T> GetPool<T>() where T : struct
        {
            ...
            if (pool[typeID] == null)
            {
                pool[typeID] = new ComponentPool<T>(POOL_SIZE, this) as IComponentPool;
            }
